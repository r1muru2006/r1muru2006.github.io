(() => {
  ...
  function loadScripts(scripts, index) {
    if (index < scripts.length) {
      ...
      (src ? loadScript() : loadScript(script.text)).then(() => loadScripts(scripts, index + 1)).catch((error) => {
        console.error(
  ...
  if (window.Pjax) {
    Pjax.prototype.getElements = function() {
      ...
      for (let i = 0; i < aLinks.length; i++) {
        ...
        const currentLangIndex = i18nLanguages.findIndex(
          (lang) => currentPathname.startsWith(lang.Lang)
        );
        if (currentLangIndex > -1) {
          const absoluteLangIndex = i18nLanguages.findIndex(
            (lang) => absolutePathname.startsWith(lang.Lang)
          );
